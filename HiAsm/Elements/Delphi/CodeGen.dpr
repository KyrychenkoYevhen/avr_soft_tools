...
begin
  ...
  begin
    //Result := 'LoadResStr(''' + cgt.resAddStr(PChar(S)) + ''')';
    // Прописываем в начале строки кодировку с которой была откомпилирована программа
    Result := 'LoadResStr(''' + cgt.resAddStr(PChar(Char(DefaultSystemCodePage) + Char(DefaultSystemCodePage shr 8) + S)) + ''')';
    Exit;
  end;
  Result := '';
  IntoStr := False;
...
begin
  with cgt^ do
  {$IFDEF FPC}
    Result := 'hiCreateFont('''+fntName(f)+''', '+
              Int2Str(fntSize(f)) + ', '+
              Int2Str(fntStyle(f)) + ', '+
              Int2Str(fntColor(f)) + ', '+
              Int2Str(fntCharSet(f)) + ')';
  {$ELSE}
...
begin
  ...
  dbg := Abs(integer(cgt.isDebug(params.sdk))) *(1+cgt.ReadIntParam(PARAM_DEBUG_MODE));
  def := NewStrList;
  old_def := NewStrList;
  if dbg > 0 then
   begin
...
const non_fpc:array[0..2] of string = (
     'WebBrowser', 'Flash', 'UseActiveX'
   );

function isReadyForAdd(cgt:PCodeGenTools; var rfd:TRFD_Rec; sdk:id_sdk):boolean; cdecl;
var p:id_element;
    s,c:string;
