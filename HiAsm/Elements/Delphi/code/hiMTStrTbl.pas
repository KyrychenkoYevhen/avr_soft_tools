...
type
  ...
  private
    ...
    procedure SetColumns(const ListCol: PKOLStrList; Mode:integer);
...
// Парсирование строки параметров
  procedure parseparam(se: string);
  var
    sp: string;
  begin
    se := hiMultiBlockFind.Trim(se);
    if se = '' then exit;
    { TextBkColor }        
    sp := fparse(se, ',');
    ...
    begin
      { TextColor }
      sp := fparse(se, ',');
      if sp <> '' then TextColor := str2int(sp);
      if se <> '' then
      begin
        { FontName }
        sp := fparse(se, ',');
...
begin
  ...
  else if (Stage = CDDS_ITEMPREPAINT) then Result := CDRF_NOTIFYSUBITEMDRAW OR CDRF_NOTIFYPOSTPAINT
...
TRY
    ...
    if FColorItems and (ItemIdx <> -1) and (SubItemIdx <> -1) then
    begin
...
// Работаем с параметрами ячейки только при наличии верхней точки
      if Assigned(_data_FormatCell.Event) then
      begin
        FontName := _prop_Font.Name;    
        FontSize := _prop_Font.Size;
        FontStyle := _prop_Font.Style;
        FontCharSet := _prop_Font.CharSet;
        se := '';

// Отправка и получение данных с верхней точки
        dtString(ds, Sender.LVItems[ItemIdx, SubItemIdx]);
        dtInteger(drow, ItemIdx);
        dtInteger(dcol, SubItemIdx);      
        ds.ldata := @drow;
        drow.ldata := @dcol;
        _ReadData(ds, _data_FormatCell);
        se := Share.ToString(ds);

// Парсирование строки параметров
        if se <> '' then parseparam(se);

// Инициализация нового шрифта
        if Assigned(GFont) then GFont.free;
        GFont := NewFont;
        Share.SetFont(GFont, FontStyle);
        GFont.FontName:= FontName;
        GFont.FontHeight:= _hi_SizeFnt(FontSize);
        GFont.FontCharset:= FontCharSet;
        SelectObject(DC, GFont.Handle);	
      end;
    end;

    if Assigned(_prop_DrawManager) then
      Result := _prop_DrawManager.customdraw(Sender, DC, Stage, ItemIdx, SubItemIdx, Rect,
                                             ItemState, TextColor, BackColor, NewCurIdx, SmIList, StIList)
EXCEPT
END;
  end;
end;
...
begin
  ...
  begin
    case Msg.message of
      ...
      begin
        Pt:= TMouseDownPt(Integer(Msg.lParam));
        HTI.pt.x := Pt.X;
        HTI.pt.y := Pt.Y;
        Sender.Perform(LVM_SUBITEMHITTEST, 0, NativeInt(@HTI));
...
function WndHDR(Sender: PControl; var Msg: TMsg; var Rslt: LRESULT): Boolean;
{$ifndef KOL3XX}
const
...
begin
  ...
  begin
    case Msg.message of
      WM_NOTIFY:
        case THDNotify(Pointer(Msg.LParam)^).Hdr.code of
        HDN_ITEMCHANGINGA, HDN_ITEMCHANGINGW:
          _hi_onEvent(_event_onChangeWidth, Sender.LVColWidth[THDNotify(Pointer(Msg.LParam)^).Item]);        
        HDN_ENDTRACKA, HDN_ENDTRACKW:
        begin
          ind := THDNotify(Pointer(Msg.LParam)^).Item;
          s := Sender.LVColText[ind] + _ColDlm +
               int2str(PHDNotify(Msg.LParam)^.pItem^.cxy) + _ColDlm +
               int2str(Sender.LVColImage[ind]) + _ColDlm +
               int2str(ord(Sender.LVColAlign[ind]));
          CList.Items[ind] := InitColStr(s);
          dtInteger(di, ind);
          dtInteger(dw, PHDNotify(Msg.LParam)^.pItem^.cxy);
          di.ldata := @dw;
...
var
  dt,ds: TData;
begin
   dtInteger(dt, Control.LVItemAtPos(Ms.X, Ms.Y));
   dtInteger(ds, NewCurIdx);
   dt.ldata := @ds;
   CopyData(@_Data, @dt);
end;
    ...
    begin 
      ...
      begin
        ...
        begin
          ...
          Control.LVItems[Row, Col] := Share.ToString(ds);
        end  
        else
