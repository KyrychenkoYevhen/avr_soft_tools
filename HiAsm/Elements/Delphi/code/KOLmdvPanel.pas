...
function WndProcmdvPanel(Sender: PControl; var Msg: TMsg; var Rslt: LRESULT): Boolean;
...
begin
    ...
    case Msg.message of
      WM_PAINT,WM_PRINTCLIENT: Begin
        {$ifndef KOL3XX}if PmdvPanel(Sender).fUpdCount>0 then Exit;{$endif}
      ...
      WM_SETTEXT: begin  //anti border-flicks after text changing 
        if lStrCmp(PChar(Msg.lParam),PChar(PmdvPanel(Sender).fCaption))<>0 then
...
begin
    ...
    if not {$IFDEF KOL3XX}(G2_Transparent in fFlagsG2){$ELSE}fTransparent{$ENDIF} then
    begin
    ...
    Windows.{$ifdef UNICODE}DrawTextW{$else}DrawTextA{$endif}(Canvas.Handle, PChar(fCaption), -1, Rect, Flags);
end;
