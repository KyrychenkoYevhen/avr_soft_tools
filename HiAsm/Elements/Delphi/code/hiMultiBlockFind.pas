...
var
  ...
  chr: Char;
  {$ifdef UNICODE}
  // В UNICODE массив будет иметь размер 256 Кб против 1 Кб в ANSI.
  // Выделять на стеке такой массив рискованно.
  BMT: array of integer;
  {$else}
...
begin
  ...
  begin
    {$ifdef UNICODE}
    SetLength(BMT, {Ord(High(Char)) + 1} $FFFF);
    {$endif}
    for i := 0 to Ord(High(Char)) do
      BMT[{$ifndef UNICODE}char{$endif}(i)] := lp;    
    for i := 1 to (lp - 1) do
      BMT[{$ifdef UNICODE}Ord{$endif}(SubStr[i])] := lp - i;
    ...
    begin
      ...
      Result := Result + BMT[{$ifdef UNICODE}Ord{$endif}(S[Result])];
    end;              
  end  
  else
    while Result <= ls do
...
begin
  ...
  begin
    dtNull(dt); // ставить нужно именно здесь, иначе происходит крэш при непервом ненайденном блоке 
    mt := nil;
    initmt := true;
...
begin
  _prop_TagList := Share.ToString(_Data);
end;
