...
uses Windows,{$ifndef KOL3XX}ShellAPI,{$endif} Kol,Share,Messages,Debug;
...
type 
  NotifyIconData_50 = record // определённая в shellapi.h 
    ...
    hIcon: HICON; 
    szTip: array[0..MAXCHAR] of Char; 
    dwState: DWORD; 
    dwStateMask: DWORD; 
    szInfo: array[0..MAXBYTE] of Char; 
...
begin
   Shell_NotifyIcon(NIM_DELETE,PNotifyIconData(@FTrayData));
   FInst := false;
end;
...
Begin
  ...
  if FInst = false then 
    begin
        ...
        Shell_NotifyIcon(NIM_ADD,PNotifyIconData(@FTrayData));
        FInst := true;
    end;
End;
...
begin
  FHint := Share.ToString(_data);
  StrLCopy(FTrayData.szTip,PChar(FHint),SizeOf(FTrayData.szTip)-1);
  Shell_NotifyIcon(NIM_MODIFY,PNotifyIconData(@FTrayData));
end;
...
begin 
  ...
  Shell_NotifyIcon(NIM_MODIFY, PNotifyIconData(@NID_50)); 
end;
