...
var   objService : OLEVariant;
      objMemory : OLEVariant;
      colMemory : OLEVariant;
      oEnum : IEnumVariant;
      iValue : LongWord;
      sComputer : string;
begin
  ...
  iValue := 0;
  while oEnum.Next(1,objMemory,{$ifndef FPC_NEW}@{$endif}iValue) = 0 do 
  begin
...
begin
  ...
  colMemory := objService.ExecQuery(WideString('SELECT ' + sQuery + ' FROM Win32_PhysicalMemory' + sDevice));
  oEnum := IUnknown(colMemory._NewEnum) as IEnumVariant;
  iValue := 0;
  while oEnum.Next(1,objMemory,{$ifndef FPC_NEW}@{$endif}iValue) = 0 do 
  begin
  {$ifdef FPC_NEW}
    try BankLabel      := Trim(VarToStr(objMemory.BankLabel)); except BankLabel := '' end;
    try Name           := Trim(VarToStr(objMemory.Name)); except Name := '' end;
    try Capacity       := Trim(VarToStr(objMemory.Capacity)); except Capacity := '' end;
    try Manufacturer   := Trim(VarToStr(objMemory.Manufacturer)); except Manufacturer := '' end;
    try
      if not VarIsEmpty(objMemory.FormFactor) then 
      begin
    ...
    try MemoryType     := Trim(VarToStr(objMemory.MemoryType)); except MemoryType := '' end;
    try Model          := Trim(VarToStr(objMemory.Model)); except Model := '' end;
    try SerialNumber   := Trim(VarToStr(objMemory.SerialNumber)); except SerialNumber := '' end;
    try Speed          := Trim(VarToStr(objMemory.Speed)); except Speed := '' end;
    try Tag            := Trim(VarToStr(objMemory.Tag)); except Tag := '' end;
    try Version        := Trim(VarToStr(objMemory.Version)); except Version := '' end;
    try DataWidth      := Trim(VarToStr(objMemory.DataWidth)); except DataWidth := '' end;
