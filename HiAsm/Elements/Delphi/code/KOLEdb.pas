...
type
  ...
  tagVariant = packed Record
    ...
    10:( pdecVal    : {$ifdef FPC_NEW}PDecimal{$else}^Decimal{$endif} );
    end;
...
function StringToOleStr(const S: string): WideString; {$ifndef VER120}inline;{$endif}
begin
  Result := WideString(S);
end;
...
begin
  ...
  if (fRowSet = nil) or (fCurIndex < 0) or (DWORD(Idx) >= ColCount) then
    Exit;
  P := Pointer( NativeUInt( fRowBuffers.Items[ fCurIndex ] ) +
...
begin
  ...
  if (fRowSet = nil) or (fCurIndex < 0) or (DWORD(Idx) >= ColCount) then
    Exit;
  dt := PDataStruct(NativeUInt(fRowBuffers.Items[fCurIndex]) + fBindings[Idx].obStatus);
  if dt.Status and $03 > 0 then
    exit;
  P := Pointer( NativeUInt( fRowBuffers.Items[ fCurIndex ] ) + fBindings[ Idx ].obValue );
  {$ifdef _db_debug_}
