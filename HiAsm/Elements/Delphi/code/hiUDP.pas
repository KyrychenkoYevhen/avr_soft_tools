...
begin
  ...
  SockAddr.sin_addr.S_addr := inet_addr(PAnsiChar(AnsiString(ReadString(_Data,_data_LocalIP,_prop_LocalIP))));
  ...
  if _prop_BroadCast then
   begin
    bc := SO_BROADCAST;
    if setsockopt(Handle,SOL_SOCKET,SO_BROADCAST,Pointer(@bc),SizeOf(bc)) <> 0 then
     ;
   end;

  if _prop_AutoConnect then
     begin
      SockAddr.sin_addr.S_addr := inet_addr(PAnsiChar(AnsiString(ReadString(_Data,_data_RemoteIP,_prop_RemoteIP))));
...
function THIUDP.Execute(Sender:PThread): NativeInt;
    ...
    begin
     ...
     if (select(0,@FDSet,nil,nil,nil) > 0) {and FD_ISSET(Handle, FDSet)} then
      begin
       ...
       SetLength(ResData,BinaryStrSize(len));
       ln := sizeof(sockaddr_in);
       len := recvfrom(Handle,ResData[1],len,0,sc,ln);
       if len = 0 then continue;
       setlength(ResData,BinaryStrSize(len));
       PadBinaryBuf(Pointer(ResData), len);
       ...
       new(dt);
       {$ifndef FPC_NEW}cardinal(dt^) := 0;{$endif} // Зачем это? (NetSpirit)
       dt^ := ResData;
...
begin
   ...
   send(Handle,s[1],BinaryLength(s),0);
end;
...
begin
   ...
   if s = '' then
     SockAddr.sin_addr.S_addr := Integer(INADDR_BROADCAST)
   else
     SockAddr.sin_addr.S_addr := inet_addr(PAnsiChar(AnsiString(s)));

   s := ReadString(_Data,_data_Data,'');
   sendto(Handle,s[1],BinaryLength(s),0,SockAddr,sizeof(TSockAddr));
end;
