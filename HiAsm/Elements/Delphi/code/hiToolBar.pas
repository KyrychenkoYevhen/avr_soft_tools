...
type
  THIToolBar = class(THIWin)
   private
    ...
    procedure SetBtns(Value:PKOLStrListEx);
   ...
   public
    ...
    property _prop_Buttons:PKOLStrListEx write SetBtns;
  end;
...
var
  ...
  dtidx, dtpos: TData;
begin
  r := Control.TBButtonRect[Control.{$IFDEF KOL3XX}TBCurItem{$ELSE}CurItem{$ENDIF}]; // В новом KOL CurItem нет.
  pos.x := r.left;
  pos.y := r.bottom;
  pos := Control.Client2Screen(pos);
  dtInteger(dtidx, Control.{$IFDEF KOL3XX}TBCurItem{$ELSE}CurItem{$ENDIF}mod 100);
