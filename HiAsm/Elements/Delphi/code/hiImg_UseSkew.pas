...
begin
    ...
    if nTransform = nil then
     begin
      ...
      Result := False;                            //не изменять координаты
     end
    else 
     begin
      Result := nTransform._Set(pDC,x1,y1,x2,y2); //следующая трансформация
      GetWorldTransform(pDC,nXFORM);              //получает установленную трансформацию
      CombineTransform({$IFDEF FPC_NEW}@{$ENDIF}mXFORM,nXFORM,mXFORM);     //комбинирует полученную трансформацию с текущей
