...
const
  ID_PLISTEDIT = 'PLISTEDIT';



function ListWindowProc(Sender: PControl; var Msg: TMsg; var Rslt: LRESULT ): Boolean;
var
  M: TMessage;
  P: PListEdit;
begin
  P := PListEdit(GetProp(Sender.Handle, ID_PLISTEDIT));
  if P <> nil then
  begin
    M.Msg := Msg.message;
    M.wParam := Msg.wParam;
...
begin
   ...
   {$ifdef KOL3XX}
    SetProp(Result.Handle, ID_PLISTEDIT, THandle(P));
   {$endif}
...
begin
  ...
  {$ifdef KOL3XX}
    //LView.DetachProc(ListWindowProc);
  {$else}
   SetWindowLongPtr(LView.Handle, GWLP_WNDPROC, OldWind);
...
begin
  ...
  TabStrt            := False;
  {$ifdef KOL3XX}
    LView.AttachProc(ListWindowProc);
  {$else}
    OldWind := GetWindowLongPtr(LV.Handle, GWLP_WNDPROC);
    NewWind := NativeInt(MakeObjectInstance(NewWndProc));
    SetWindowLongPtr(LV.Handle, GWLP_WNDPROC, NewWind);
  {$endif}
end;
