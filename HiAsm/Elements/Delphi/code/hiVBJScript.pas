...
type
  ...
  private
    ...
    function ReadPointIndex(List:PKOLStrList; Data:PData):integer;
...
begin
  if FArray.vt = VT_DISPATCH then begin
    DataToOle(el,Val);
    CallDispatch(IDispatch(FArray.dispVal), Share.ToString(Item), DISPATCH_PROPERTYPUT, nil, [el]);
...
begin
  ...
  if FArray.vt = VT_DISPATCH then
    Result := CallDispatch(IDispatch(FArray.dispVal), Share.ToString(Item), DISPATCH_PROPERTYGET, @el, []);
