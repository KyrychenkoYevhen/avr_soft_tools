...
begin
  ...
  GetEncoderClsid(WideString(ftype), encoderClsid);//получаем GUID нужного нам енкодера
  ...
  begin
    ...
    if Method = _FILE then
      imgFile.Save(WideString(aFile), encoderClsid, @encoderParameters)//все данные настройки
                       //заданы передаем их в функцию Save для сохранения в файл
    else
      imgFile.Save(aptrout, encoderClsid, @encoderParameters);                     
  end
  else if Method = _FILE then
    imgFile.Save(WideString(aFile), encoderClsid) //если используем дефолтные настройки то 
