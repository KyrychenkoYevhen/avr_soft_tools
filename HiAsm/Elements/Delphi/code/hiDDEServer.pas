...
var g_RegisteredServices:PKOLStrList;
...
begin
  ...
  XTYP_DISCONNECT:
   ...
   begin
    dwLen := DdeQueryString(g_DdeInstance,hSz1,nil,0,DDE_CODEPAGE)+1; SetLength(s,dwLen);
    DdeQueryString(g_DdeInstance,hSz1,@s[1],dwLen,DDE_CODEPAGE); SetLength(s,dwLen-1);
    ...
    XTYP_EXECUTE:
     begin
       ...
       dwLen := DdeGetData(hData,nil,0,0) div SizeOf(Char); SetLength(s,dwLen);
     ...
     begin
      ...
      XTYP_ADVREQ,XTYP_REQUEST:
       begin
         ...
         if not FError then Result := DDEML.DdeCreateDataHandle(g_DdeInstance,PChar(FValue),Length(FValue){+1} * SizeOf(Char),0,hSz2,CF_TEXT,0);
       end;
      XTYP_POKE:
       begin
         dwLen := DdeGetData(hData,nil,0,0) div SizeOf(Char); SetLength(s, dwLen);
...
begin
  ...
  if s='' then hszItem := 0 else
    hszItem := DdeCreateStringHandle(g_DdeInstance,PChar(s),DDE_CODEPAGE); 
  s := ReadString(_Data,_data_ChangedTopic,'');
  if (s='') and (FTopicList.Count=1) then s := FTopicList.Items[0]; 
  if s='' then hszTopic := 0 else
    hszTopic := DdeCreateStringHandle(g_DdeInstance,PChar(s),DDE_CODEPAGE); 
...
begin
  FTopicList.Text := Share.ToString(_Data);
end;

initialization
  g_RegisteredServices := NewKOLStrList;
