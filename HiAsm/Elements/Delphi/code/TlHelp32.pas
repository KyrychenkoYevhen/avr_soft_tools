...
{$EXTERNALSYM tagPROCESSENTRY32}
  tagPROCESSENTRY32A = packed record
    dwSize: DWORD;
    cntUsage: DWORD;
    th32ProcessID: DWORD;       // this process
    th32DefaultHeapID: DWORD;
...
begin
  if InitToolHelp then
    Result := {$ifdef UNICODE}_Process32FirstW{$else}_Process32First{$endif}(hSnapshot, lppe)
  else Result := False;
end;
...
begin
  if InitToolHelp then
    Result := {$ifdef UNICODE}_Process32NextW{$else}_Process32Next{$endif}(hSnapshot, lppe)
  else Result := False;
end;
...
begin
  if InitToolHelp then
    Result := {$ifdef UNICODE}_Module32FirstW{$else}_Module32First{$endif}(hSnapshot, lpme)
  else Result := False;
end;
...
begin
  if InitToolHelp then
    Result := {$ifdef UNICODE}_Module32NextW{$else}_Module32Next{$endif}(hSnapshot, lpme)
  else Result := False;
end;
