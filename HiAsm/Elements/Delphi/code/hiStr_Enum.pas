...
function FParse(var S: string; const Delimiter: Char): string;
function RParse(var S: string; const Delimiter: Char): string;
...
function StrRScanLen(Str: PWideChar; Chr: WideChar; Len: Integer): PWideChar; overload;
begin
  Result := Str - Len;
  while Len > 0 do
  begin
...
{$IFDEF WIN64}
function StrRScanLen(Str: PAnsiChar; Chr: AnsiChar; Len: Integer): PAnsiChar; overload;
begin
  Result := Str - Len;
  while Len > 0 do
  begin
...
{$ELSE}
function StrRScanLen(Str: PAnsiChar; Chr: AnsiChar; Len: Integer): PAnsiChar; assembler; overload;
asm
        PUSH    EDI
        XCHG    EDI, EAX
        XCHG    EAX, EDX
...
begin
  ...
  L := S-F-byte((F+1)^=Dlm); 
  SetLength(Result, L);
  Move((S-L+1)^, Result[1], L*SizeOf(Char));
  S := F;
end;
...
begin
  Q := PChar(S);
  P := Q + Length(S) - 1;
  F := StrRScanLen(P, Delimiter, Length(S));
  L := P - F - byte((F + 1)^ = Delimiter); 
  SetLength(Result, L);
  Move((P - L + 1)^, Result[1], L*SizeOf(Char));
  SetLength(S, F - Q + 1); 
end;
//------------------------------------------------------------------------------
