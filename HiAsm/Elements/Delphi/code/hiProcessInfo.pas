...
begin
  ...
  iValue := 0;
  while oEnum.Next(1,objProcess,{$ifndef FPC_NEW}@{$endif}iValue) = 0 do
  begin
...
begin
  ...
  colProcess := objService.ExecQuery(WideString('SELECT ' + sQuery + ' FROM Win32_Process' + sDevice));
  oEnum := IUnknown(colProcess._NewEnum) as IEnumVariant;
  iValue := 0;
  while oEnum.Next(1,objProcess,{$ifndef FPC_NEW}@{$endif}iValue) = 0 do
  begin
  {$ifdef FPC_NEW}
    try ProcessID         := Trim(VarToStr(objProcess.ProcessID)); except ProcessID := '' end;
    try ParentProcessID   := Trim(VarToStr(objProcess.ParentProcessID)); except ParentProcessID := '' end;
    try Name              := Trim(VarToStr(objProcess.Name)); except Name := '' end;
    try ExecutablePath    := Trim(VarToStr(objProcess.ExecutablePath)); except ExecutablePath := '' end;
    try CommandLine       := Trim(VarToStr(objProcess.CommandLine)); except CommandLine := '' end;
    ...
    try Priority          := Trim(VarToStr(objProcess.Priority)); except Priority := '' end;
    try ThreadCount       := Trim(VarToStr(objProcess.ThreadCount)); except ThreadCount := '' end;
    try OSName            := Trim(VarToStr(objProcess.OSName)); except OSName := '' end;
    try WindowsVersion    := Trim(VarToStr(objProcess.WindowsVersion)); except WindowsVersion := '' end;
