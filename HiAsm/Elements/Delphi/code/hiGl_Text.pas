...
begin
  glPushAttrib(GL_ALL_ATTRIB_BITS);
   ...
   //glColor(_prop_Font.Color);
   Litera := ReadString(_Data, _data_Text, _prop_Text);
   glListBase(_prop_ListStart);
   // Национальные Unicode символы таким образом не отображаются
   //glCallLists(Length(Litera), {$ifdef UNICODE}GL_UNSIGNED_SHORT{$else}GL_UNSIGNED_BYTE{$endif}, Pointer(Litera));
   glCallLists(Length(Litera), GL_UNSIGNED_BYTE, Pointer(Litera));
   //glListBase(0);
  glPopAttrib;
