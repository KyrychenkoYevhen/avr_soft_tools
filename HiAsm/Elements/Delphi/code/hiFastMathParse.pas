...
uses Kol, {$ifdef ERR_UNIT} err, {$else} sysutils, {$endif} Share, Debug;
...
type
  ...
  THIFastMathParse = class(TDebug)
   private
    ...
    procedure ProcCmd0(cmd: AnsiString; mnem: string);
    procedure ProcCmd1(i: integer; cmd: AnsiString; mnem: string);
    procedure ProcCmd2(var x: real; cmd: AnsiString; mnem: string);
    procedure ListCmd1(var L: ListArr; i: integer; cmd: AnsiString; mnem: string);
    procedure GetToken;
...
begin
  if LPos >= 0 then dtNull(_Data)
  else
    if _prop_ResultType = 0 then dtInteger(_Data, Round(FResult))
    else
      dtReal(_Data, FResult);
end;

procedure THIFastMathParse._var_Proc;
begin
  dtString(_Data, {$ifdef UNICODE}BufToBinaryStr(Pointer(Proc), Length(Proc)){$else}Proc{$endif});
end;
  ...
  try
    ...
    for i:=1 to FDataCount do
      if IndV[i]>=0 then begin
        ...
        {$ifndef ERR_UNIT}
          //raise Exception.Create('MathParse error: invalid argument');
    ...
    FResult := Tproc(Pointer(Proc))(self,Vars);
    if _prop_ResultType=0 then Res:=Round(FResult);
    LPos := -1;
  except on E:Exception do
    {$ifndef ERR_UNIT}
    if E.Message = '0' then Err := 0 else Err := 1;
    {$else}
    case E.Code of
    ...
    begin
      ...
      begin//x := FResult
        ...
        if i<=FDataCount then
          if Assigned(self.X[i-1].Event) then
          begin
            j:=IndV[i];
            if j<0 then
            begin
...
procedure THIFastMathParse.ProcCmd2(var x: real; cmd: AnsiString; mnem: string);
var i:integer;
begin
  i:=high(Vars)+1;
  SetLength(Vars,i+1);
...
procedure THIFastMathParse.ProcCmd1(i: integer; cmd: AnsiString; mnem: string);
var s:string[4];
begin
  SetLength(s,1);
...
procedure THIFastMathParse.ProcCmd0(cmd: AnsiString; mnem: string);
var i:integer;
    b:byte;
    s:string;
begin
...
procedure THIFastMathParse.ListCmd1(var L: ListArr; i: integer; cmd: AnsiString; mnem: string);
var s:string[4];
begin
  SetLength(s,1);
  integer(pointer(@s[1])^):=i;
