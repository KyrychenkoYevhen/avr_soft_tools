...
// TODO: почему не используются GetMem/FreeMem вместо этих самодельных функций?
function AllocMemo(Size: Longint): Pointer;
{$ifdef FPC}var   HeapAllocFlags: Word;{$endif}
begin
   {$ifdef FPC} HeapAllocFlags := GMEM_MOVEABLE;{$endif}
   if Size > 0 then
...
const
  ...
  CHR_EXT_INTRODUCER: AnsiChar    = '!';
  CHR_IMAGE_SEPARATOR: AnsiChar   = ',';
  CHR_TRAILER: AnsiChar           = ';';  { indicates the end of the GIF Data stream }
...
{$ifdef WIN64}
function FillMaskLine4( Mask, Scan : PByte; W : Integer; TransIdx : Integer ): Boolean;
begin
  Result := False;
end;
{$else}
...
{$ifdef WIN64}
function FillMaskLine8( Mask, Scan : PByte; W : Integer; TransIdx : Integer ): Boolean;
begin
  Result := False;
end;
{$else}
...
{$ifdef WIN64}
function FillMaskLine0( Mask, Scan : PByte; W : Integer ): Boolean;
begin
  Result := False;
end;
{$else}
...
{$ifdef WIN64}
function FillMaskLine1( Mask, Scan : PByte; W : Integer ): Boolean;
begin
  Result := False;
end;
{$else}
...
var   SeparatorChar: AnsiChar;
   ...
   begin
      ...
      for I := Low(TGIFVersion) to High(TGIFVersion) do
         if CompareMem( @S[ 0 ], GifVersionStr[ I ], 4 ) then
         begin
            FVersion := I;
            Break;
         end;
   ...
   function ReadDataBlock(Stream: PStream): PKOLStrList;
   var   BlockSize: Byte;
         S: string;
   begin
   ...
   function ReadExtensionBlock(Stream: PStream; var SeparatorChar: AnsiChar): PList;
...
begin
   FGifData.DComment.Free;
   FGifData.DComment := NewKOLStrList;
