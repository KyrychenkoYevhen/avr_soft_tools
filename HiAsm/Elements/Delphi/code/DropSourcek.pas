...
type
  ...
  protected
    ...
    function QueryContinueDrag(fEscapePressed: bool;
      grfKeyState: {$ifdef FPC_NEW}DWORD{$else}LongInt{$endif}): HRESULT; stdcall;
    function GiveFeedback(dwEffect: {$ifdef FPC_NEW}DWORD{$else}LongInt{$endif}): HRESULT; stdcall;
    ...
    function EnumFormatEtc(dwDirection: {$ifdef FPC_NEW}DWORD{$else}LongInt{$endif};
    ...
    {$endif}  
    function dUnadvise(dwConnection: {$ifdef FPC_NEW}DWORD{$else}LongInt{$endif}): HRESULT; stdcall;
...
{ record for returning strings from IShellFolder member functions }
  ...
  PFileDescriptor = {$ifdef UNICODE}PFileDescriptorW{$else}PFileDescriptorA{$endif};
  ...
  _FILEDESCRIPTOR = {$ifdef UNICODE}_FILEDESCRIPTORW{$else}_FILEDESCRIPTORA{$endif};

  TFileDescriptorA = _FILEDESCRIPTORA;
  TFileDescriptorW = _FILEDESCRIPTORW;
  TFileDescriptor = {$ifdef UNICODE}TFileDescriptorW{$else}TFileDescriptorA{$endif};

  FILEDESCRIPTORA = _FILEDESCRIPTORA;
  FILEDESCRIPTORW = _FILEDESCRIPTORW;
  FILEDESCRIPTOR = {$ifdef UNICODE}FILEDESCRIPTORW{$else}FILEDESCRIPTORA{$endif};

{ format of CF_FILEGROUPDESCRIPTOR }

  PFileGroupDescriptorA = ^TFileGroupDescriptorA;
  PFileGroupDescriptorW = ^TFileGroupDescriptorW;
  PFileGroupDescriptor = {$ifdef UNICODE}PFileGroupDescriptorW{$else}PFileGroupDescriptorA{$endif};
  ...
  _FILEGROUPDESCRIPTOR = {$ifdef UNICODE}_FILEGROUPDESCRIPTORW{$else}_FILEGROUPDESCRIPTORA{$endif};

  TFileGroupDescriptorA = _FILEGROUPDESCRIPTORA;
  TFileGroupDescriptorW = _FILEGROUPDESCRIPTORW;
  TFileGroupDescriptor = {$ifdef UNICODE}TFileGroupDescriptorW{$else}TFileGroupDescriptorA{$endif};

  FILEGROUPDESCRIPTORA = _FILEGROUPDESCRIPTORA;
  FILEGROUPDESCRIPTORW = _FILEGROUPDESCRIPTORW;
  FILEGROUPDESCRIPTOR = {$ifdef UNICODE}FILEGROUPDESCRIPTORW{$else}FILEGROUPDESCRIPTORA{$endif};
  
  {$ifdef F_P}TDragInfo = TDragInfoA;{$endif }
  {$ifdef DELPHI_UNI}TDragInfo = _DRAGINFO;{$endif }
...
begin
  ...
  try
    ...
    if (DesktopFolder.ParseDisplayName(0,
          nil,PWideChar(WideString(FolderPath))
  ,dummy,PathPidl,pdwAttributes) = NOERROR) and
...
begin
  ...
  try
    if (SHGetDesktopFolder(DeskTopFolder) = NOERROR) then
      DesktopFolder.ParseDisplayName(0,nil,PWideChar(WideString(Path)),dummy1,result,dummy2);
  except
  end;
end;
// -----------------------------------------------------------------------------
...
begin
  result := nil;
  try
    Folder.ParseDisplayName(0,nil,PWideChar(WideString(Sub)),dummy1,result,dummy2);
  except
  end;
end;
// -----------------------------------------------------------------------------
...
function TInterfacedComponent.QueryInterface({$IFDEF FPC_HAS_CONSTREF}constref{$ELSE}const{$ENDIF} IID: TGuid; out Obj): HRESULT;
begin
...
{$ifdef FPC_NEW}
function TEnumFormatEtc.Next(Celt:ULong;
  out Elt:FormatEtc;pceltFetched:pULong=nil):HResult; StdCall;
{$else}  
...
{$ifdef FPC_NEW}
function TEnumFormatEtc.Skip(Celt: ULong): HRESULT;
{$else}  
...
function TDropSource.GiveFeedback(dwEffect: {$ifdef FPC_NEW}DWord{$else}LongInt{$endif}): HRESULT; stdcall;
var
...
{$ifdef FPC_NEW}
function TDropSource.dAdvise(const FormatEtc: TFormatEtc; advf: DWORD;
  const advsink: IAdviseSink; out dwConnection: DWORD): HRESULT; stdcall;
{$else}  
...
function TDropSource.DUnadvise(dwConnection: {$ifdef FPC_NEW}DWORD{$else}LongInt{$endif}): HRESULT;
begin
  result := OLE_E_ADVISENOTSUPPORTED;
end;
// -----------------------------------------------------------------------------
...
function TDropSource.EnumFormatEtc(dwDirection: {$ifdef FPC_NEW}DWORD{$else}LongInt{$endif};
  out EnumFormatEtc:IEnumFormatEtc): HRESULT; stdcall;
begin
...
begin
  ...
  res := DoDragDrop(Self as IDataObject, Self as IDropSource, okeffect, {$ifdef FPC_NEW}@effect{$else}effect{$endif});
...
begin
  ...
  Medium.tymed := 0;
  Medium.{$ifdef FPC_NEW}PUnkForRelease{$else}UnkForRelease{$endif} := NIL;
  Medium.hGlobal := 0;
...
var
  i: Integer;
  dropfiles: {$ifdef FPC_NEW}LPDRAGINFO{$else}PDragInfo{$endif};
  pFile: PChar;
  pFileW: PWideChar;
  DropEffect: ^DWORD;
  strlength: Integer;
  tmpFilenames: PKOLStrList;
begin
  Medium.tymed := 0;
  Medium.{$ifdef FPC_NEW}PUnkForRelease{$else}UnkForRelease{$endif} := NIL;
  Medium.hGlobal := 0;
  ...
  //--------------------------------------------------------------------------
    ...
    (FormatEtcIn.tymed and TYMED_HGLOBAL <> 0) then
  begin
    // Подсчитывается общая длина имён в списке
    strlength := 0; // strlength - в символах
    for i := 0 to Files.Count-1 do
      Inc(strlength, Length(Files.Items[i])+1); // +1: завершающий #0 каждой строки
    // Выделяется память на размер TDragInfo + (длина имён + #0) + дополнительный завершающий #0
    Medium.hGlobal :=
      GlobalAlloc(GMEM_SHARE or GMEM_ZEROINIT, SizeOf(TDragInfo) + (strlength+1)*SizeOf(Char));
    if (Medium.hGlobal = 0) then
      result:=E_OUTOFMEMORY
    else
    begin
      ...
      try
        dropfiles^.uSize := SizeOf(TDragInfo);
        dropfiles^.fNC := False;
        NativeUInt(pFile) := NativeUInt(dropfiles)+SizeOf(TDragInfo); // Позиция имён в выделенном участке (после TDragInfo)
        // Перемещение имён в выделенный участок
        for i := 0 to Files.Count-1 do
        begin
  ...
  //--------------------------------------------------------------------------
  else if (FormatEtcIn.cfFormat = CF_FILENAMEMAP) and // TODO: Format ANSI - need convert to AnsiString on UNICODE mode
    (FormatEtcIn.dwAspect = DVASPECT_CONTENT) and
  ...
  begin
    ...
    Medium.hGlobal :=
      GlobalAlloc(GMEM_SHARE or GMEM_ZEROINIT, (strlength+1)*SizeOf(Char));
    if (Medium.hGlobal = 0) then
      result:=E_OUTOFMEMORY
    else
    begin
  ...
  begin
    ...
    Medium.hGlobal := GlobalAlloc(GMEM_SHARE or GMEM_ZEROINIT, (strlength+1)*SizeOf(WideChar));
    if (Medium.hGlobal = 0) then
      result:=E_OUTOFMEMORY
    else
    begin
      ...
      try
        ...
        begin
          ...
          StringToWideChar(MappedNames.Items[i], pFileW,
            (length(MappedNames.Items[i])+1)*SizeOf(WideChar));
          {$else}
          StrPCopy(pFileW, MappedNames.Items[i]);
          {$endif}
