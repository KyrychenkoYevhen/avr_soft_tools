  ...
  fn := ReadString(_data,_data_RemoteName,_prop_RemoteName);
{
  ...
  if _IsStream(_data) then
    ...
    st := NewWriteFileStream(Share.ToString(_Data));
...
begin
  ...
  if _IsStream(dt) then
    st := ToStream(dt)
  else st := NewReadFileStream(Share.ToString(dt));
...
begin
  _prop_Directory := Share.ToString(_Data);
  if not FtpSetCurrentDirectory(hFTP, PChar(_prop_Directory)) then
    _hi_onEvent(_event_onError,3);
end;
...
begin
  if not FtpCreateDirectory(hFTP, PChar(Share.ToString(_Data))) then
    _hi_onEvent(_event_onError,6);
end;

procedure THIWinFTP._work_doRemoveDirectory;
begin
  if not FtpRemoveDirectory(hFTP, PChar(Share.ToString(_Data))) then
