...
begin
  ...
  Fo.Wnd    := ReadHandle;
  Fo.wFunc  := func;
  //Although this member is declared as a null-terminated string,
  //it is used as a buffer to hold multiple file names. Each file name must 
  //be terminated by a single NULL character. An additional NULL character 
  //must be appended to the end of the final name to indicate the end of pFrom.
  From := FromFolder + #0;
  Fo.pFrom  := Pointer(From);
  Fo.pTo    := PChar(ToFolder); //куда будет скопирована директори€
  Fo.fFlags := flags;
