...
begin
  ...
  iValue := 0;
  while oEnum.Next(1,objLogicalDisk,{$ifndef FPC_NEW}@{$endif}iValue) = 0 do 
  begin
...
begin
  ...
  colLogicalDisk := objService.ExecQuery(WideString('Select ' + sQuery + ' from Win32_LogicalDisk' + sDevice));
  oEnum := IUnknown(colLogicalDisk._NewEnum) as IEnumVariant;
  iValue := 0; 
  ...
  begin
  {$ifdef FPC_NEW}
    try DeviceID     := Trim(VarToStr(objLogicalDisk.DeviceID)); except DeviceID := '' end;
    try Description  := Trim(VarToStr(objLogicalDisk.Description)); except Description := '' end;
    try FileSystem   := Trim(VarToStr(objLogicalDisk.FileSystem)); except FileSystem := '' end;
    try DriveType    := Trim(VarToStr(objLogicalDisk.DriveType)); except DriveType := '' end;
    try FullSize     := Trim(VarToStr(objLogicalDisk.Size)); except FullSize := '' end;
    try FreeSpace    := Trim(VarToStr(objLogicalDisk.FreeSpace)); except FreeSpace := '' end;
    if (FullSize <> '') and (FreeSpace <> '') then
      try 
       ...
       OccupiedSpace   := ''; 
    try VolumeName         := Trim(VarToStr(objLogicalDisk.VolumeName)); except VolumeName := '' end;
    try VolumeSerialNumber := Trim(VarToStr(objLogicalDisk.VolumeSerialNumber)); except VolumeSerialNumber := '' end;
