...
uses Windows,Kol,Share,mmsystem,Debug{,Media};
...
begin
  ...
  with OpenParm do
   begin
    ...
    if mciSendCommand(0, MCI_OPEN, MCI_OPEN_TYPE or MCI_OPEN_ELEMENT, NativeUInt(@OpenParm)) = 0 then
       mciSendCommand(OpenParm.wDeviceID, MCI_SET, MCI_SET_DOOR_OPEN, 0);
    mciSendCommand(OpenParm.wDeviceID, mci_Close, mci_Open_Type or mci_Open_Element, NativeUInt(@OpenParm));
   end;
end;
   ...
   begin
    ...
    if mciSendCommand(0, MCI_OPEN, MCI_OPEN_TYPE or MCI_OPEN_ELEMENT, NativeUInt(@OpenParm)) = 0 then
      begin
        ...
        mciSendCommand(FID, MCI_PLAY,0, NativeUInt(@pp));
      end;
   end;
end;
...
begin
  ...
  with OpenParm do
   begin
    ...
    mciSendCommand(FID, MCI_CLOSE, mci_Open_Type or mci_Open_Element, NativeUInt(@OpenParm));
    {
    dwCallback := 0;
    lpstrDeviceType := 'CDAudio';
    lpstrElementName := PChar(FCDROM);
    if mciSendCommand(0, MCI_OPEN, MCI_OPEN_TYPE or MCI_OPEN_ELEMENT, NativeUInt(@OpenParm)) = 0 then
      begin
        ...
        pp.dwFrom := 0;
        pp.dwTo := 0;
        mciSendCommand(OpenParm.wDeviceID, MCI_PLAY,0, NativeUInt(@pp));
      end;
    mciSendCommand(OpenParm.wDeviceID, mci_Close, mci_Open_Type or mci_Open_Element, NativeUInt(@OpenParm));
    }
   end;
