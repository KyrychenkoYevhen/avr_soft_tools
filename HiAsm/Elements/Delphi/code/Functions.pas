...
uses
  Windows, MMsystem, Messages, KOL, Share, Script;
...
begin
  PlaySound(PChar(Functions.ToString(Args[0])),0,1);
end;
...
begin
   ...
   Result.sdata := GetTime(Functions.ToString(Args[0]));
end;
...
begin
   ...
   AssignFile(f^,Functions.ToString(Args[0]));
...
begin
   f := pointer(ToInteger(Args[0]));
   if IsFHandle(cardinal(f)) then
     WriteLn(f^,Functions.ToString(Args[1]))
   else ;
end;
...
begin
  Result.Data_type := data_int;
  Result.idata := integer(FileExists(Functions.ToString(Args[0])));
end;
...
begin
   Result.Data_type := data_int;
   Result.idata := Pos(Functions.ToString(Args[0]),Functions.ToString(Args[1]));
end;

function TFunctions._Copy;
begin
   Result.Data_type := data_str;
   Result.sdata := copy(Functions.ToString(Args[0]),ToInteger(Args[1]),ToInteger(Args[2]));
end;

function TFunctions._Delete;
begin
   Result.Data_type := data_str;
   Result.sdata := Functions.ToString(Args[0]);
...
begin
   Result.Data_type := data_str;
   Result.sdata := Functions.ToString(Args[0]);
   Replace(Result.sdata,Functions.ToString(Args[1]),Functions.ToString(Args[2]));
end;

function TFunctions._Len;
begin
   Result.Data_type := data_int;
   Result.idata := Length(Functions.ToString(Args[0]));
end;
...
begin
   ...
   if Args[0].data_type <> data_str then
    begin
      Args[0].sdata := Functions.ToString(Args[0]);
      Args[0].data_type := data_str;
    end;
   s := Functions.ToString(Args[1]);
   if s = '' then
    s := ' ';
...
begin
   SetWindowText(ToInteger(Args[0]),PChar(Functions.ToString(Args[1])));
end;
...
procedure RunProcess(Fn, CmdLine: string; ShowParams: Integer);
var
  SI: TStartupInfo;
...
begin
  RunProcess(Functions.ToString(Args[0]), Functions.ToString(Args[1]), ToInteger(Args[2]));
end;
