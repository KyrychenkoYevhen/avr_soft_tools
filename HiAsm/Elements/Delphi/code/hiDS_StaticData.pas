...
type
  ...
  TIDS_Table = record
    ...
    columns:function(id:pointer):PKOLStrList of object; 
  ...
  THIDS_StaticData = class(TDebug)
   private
    ...
    function columns(id:pointer):PKOLStrList; 
...
function THIDS_StaticData.columns(id:pointer):PKOLStrList; 
begin
   Result := fCol;
end;
   ...
   if (idx < fCount) and (idx >= 0) and (colidx < fCol.count) and (colidx >= 0) then
    begin
      for c := 0 to idx do 
        inc(i^);
      dec(i^);  
      dtString(fData[i^][colidx], Share.ToString(value));
      Result := true;
    end
   else Result := false;
end;
