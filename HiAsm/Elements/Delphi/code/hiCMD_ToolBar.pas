...
begin
   ...
   imgs := NewKOLStrListEx;
   popups := NewKOLStrListEx;
...
var
  cmd:PCommandInfo;
begin
   if not tb.RightClick and (tb.{$IFDEF KOL3XX}TBCurItem{$ELSE}CurItem{$ENDIF} <> -1) then
     begin
...
var c:string;
    ...
    r:TRect;
begin
  r := tb.TBButtonRect[tb.{$IFDEF KOL3XX}TBCurItem{$ELSE}CurItem{$ENDIF}];
