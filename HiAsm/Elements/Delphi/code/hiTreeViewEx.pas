...
type
  THITreeViewEx = class(THIWin)
   private
    ...
    function _OnEndEdit(Sender: PControl; Item: THandle; const NewTxt: {$ifdef KOL3XX}KOL_String{$else}string{$endif}): Boolean;
    function _OnExpanding(Sender: PControl; Item: THandle; Expand: Boolean): Boolean;
    procedure LoadFromText(Lst:PKOLStrList);
    ...
    procedure SetIconsState(const value:PKOLStrListEx);
   ...
   public
    ...
    property _prop_IconsState:PKOLStrListEx write SetIconsState;
...
begin
   Control.TVItemText[Control.TVSelected] := Share.ToString(_Data);
end;
