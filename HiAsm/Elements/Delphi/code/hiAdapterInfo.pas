...
type
  THiAdapterInfo = class(TDebug)
   private
      ...
      FListId,FListIP,FListMask,FListGateway,FListDNS,FListMetric : PKOLStrList;
...
begin
  ...
  OleInit;
  InitCLSIDs;
  FListID := NewKOLStrList;
  FListIP := NewKOLStrList;
  FListMask := NewKOLStrList;
  FListGateway := NewKOLStrList;
  FListDNS := NewKOLStrList;
  FListMetric := NewKOLStrList;
end;
...
var
   objService : OLEVariant;
      objNetConfig : OLEVariant;
      colNetConfig : OLEVariant;
      oEnum : IEnumVariant;
      iValue : LongWord;
      sComputer : string;
begin
  ...
  iValue := 0;
  while oEnum.Next(1,objNetConfig,{$ifndef FPC_NEW}@{$endif}iValue) = 0 do
  begin
...
var   objService : OLEVariant;
      objNetConfig : OLEVariant;
      colNetConfig : OLEVariant;
      colLiveNetAdp  : OLEVariant;
      objLiveNetAdp  : OLEVariant;      
      colLiveNetAdp2  : OLEVariant;
      objLiveNetAdp2  : OLEVariant;      
      oEnum : IEnumVariant;
      oEnum2 : IEnumVariant;
      oEnum3 : IEnumVariant;
      iValue : LongWord;
...
begin
  ...
  colNetConfig := objService.ExecQuery(WideString('Select ' + sQuery + ' from Win32_NetworkAdapterConfiguration Where IPEnabled = True'));
  oEnum := IUnknown(colNetConfig._NewEnum) as IEnumVariant;
  colLiveNetAdp  := objService.ExecQuery(WideString('Select ' + dQuery + ' from Win32_NetworkAdapter' + sDevice));
  oEnum2 := IUnknown(colLiveNetAdp._NewEnum) as IEnumVariant; 
  iValue2 := 0;
  while oEnum2.Next(1,objLiveNetAdp,{$ifndef FPC_NEW}@{$endif}iValue2) = 0 do
  begin
  {$ifdef FPC_NEW}
    try SystemName := Trim(VarToStr(objLiveNetAdp.SystemName)); except SystemName := '' end;
    try Name := Trim(VarToStr(objLiveNetAdp.Name)); except Name := '' end;
    try ServiceName := Trim(VarToStr(objLiveNetAdp.ServiceName)); except ServiceName := '' end;
    try Manufacturer := Trim(VarToStr(objLiveNetAdp.Manufacturer)); except Manufacturer := '' end;
    try AdapterType := Trim(VarToStr(objLiveNetAdp.AdapterType)); except AdapterType := '' end;
    try DeviceID := Trim(VarToStr(objLiveNetAdp.DeviceID)); except DeviceID := '' end;
    try PNPDeviceID := Trim(VarToStr(objLiveNetAdp.PNPDeviceID)); except PNPDeviceID := '' end;
    try MACAddress := Trim(VarToStr(objLiveNetAdp.MACAddress)); except MACAddress := '' end;
  {$else}
  ...
  {$endif}
    ...
    begin
      ...
      if FIndex = Ind then
      begin
        colLiveNetAdp2  := objService.ExecQuery(WideString('Select AdapterType from Win32_NetworkAdapter Where Index = ' +  Ind));
        oEnum3 := IUnknown(colLiveNetAdp2._NewEnum) as IEnumVariant; 
        iValue3 := 0;
        while oEnum3.Next(1,objLiveNetAdp2,{$ifndef FPC_NEW}@{$endif}iValue3) = 0 do
        begin
          if Trim(VarToStr(objLiveNetAdp2.AdapterType)) <> '' then
          begin
          {$ifdef FPC_NEW}
            try DNSHostName  := Trim(VarToStr(objNetConfig.DNSHostName)); except DNSHostName := '' end;
            try DNSDomain    := Trim(VarToStr(objNetConfig.DNSDomain)); except DNSDomain := '' end;
            ...
            try WINSServer   := Trim(VarToStr(objNetConfig.WINSPrimaryServer)); except WINSServer := '' end;
            try DHCPServer   := Trim(VarToStr(objNetConfig.DHCPServer)); except DHCPServer := '' end;
...
begin
  ind := ToIntIndex(Item);
  if(ind >= 0)and(ind < FListId.Count)then
  begin
    Result:= true;
...
begin
  ind := ToIntIndex(Item);
  if(ind >= 0)and(ind < FListIP.Count)then
  begin
    Result:= true;
...
begin
  ind := ToIntIndex(Item);
  if(ind >= 0)and(ind < FListMask.Count)then
  begin
    Result:= true;
...
begin
  ind := ToIntIndex(Item);
  if(ind >= 0)and(ind < FListGateway.Count)then
  begin
    Result:= true;
...
begin
  ind := ToIntIndex(Item);
  if(ind >= 0)and(ind < FListDNS.Count)then
  begin
    Result:= true;
...
begin
  ind := ToIntIndex(Item);
  if(ind >= 0)and(ind < FListMetric.Count)then
  begin
    Result:= true;
