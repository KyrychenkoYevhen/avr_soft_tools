...
type
  ...
  THICOMEX = class(TDebug)
   private
    ...
    function ExecuteRd(Sender: PThread): NativeInt;
    function ExecuteWr(Sender: PThread): NativeInt;
...
//  if (not thwr.Suspended) then exit;
  BufferWr := Share.ToString(_Data);
  WriteFile(hFile, BufferWr[1], Length(BufferWr), Sended, @OvrWr);
  thwr.Resume;
end;

function THICOMEX.ExecuteRd(Sender: PThread): NativeInt;
var
...
function THICOMEX.ExecuteWr(Sender: PThread): NativeInt;
var
