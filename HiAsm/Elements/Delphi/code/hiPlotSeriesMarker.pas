        ...
        begin
          ...
          if i < Text.Count then
            begin
              ...
              Windows.{$ifdef UNICODE}DrawTextW{$else}DrawTextA{$endif}(Handle, PChar(Txt), Length(Txt), r, DT_CALCRECT);
              ...
              Windows.{$ifdef UNICODE}DrawTextW{$else}DrawTextA{$endif}(Handle, PChar(Txt), Length(Txt), r, 0);
            end; 
       end;
    end;
end;
...
begin
   ...
   for i := 0 to _prop_Series.Count-1 do
     begin
        ...
        TSMarkerSeries(FSeries).Text.Add(Share.ToString(Data));
     end; 
end;
