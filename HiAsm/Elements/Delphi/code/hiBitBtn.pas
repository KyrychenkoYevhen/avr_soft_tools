...
begin
  ...
  {$IFDEF FPC_NEW}Control.Enabled := _prop_Enabled;{$ENDIF}
...
begin
  ...
  with Control.Canvas{$ifndef F_P}^{$endif} do begin
    ...
    if BtnState=2 then begin
      ...
      {$ifdef UNICODE}WDrawText{$else}DrawText{$endif}(Control.Caption,r,DT_EXPANDTABS or DT_SINGLELINE or DT_CENTER or DT_VCENTER);
