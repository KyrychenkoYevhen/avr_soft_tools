...
begin
  ...
  if Result = HASH_NOERROR then
    Result := HashUpdate(@ctx, PChar(SrcStr), BinaryLength(SrcStr));
  if Result = HASH_NOERROR then
...
begin
  ...
  FileHandle := {$ifdef UNICODE}CreateFileW{$else}CreateFileA{$endif}(PChar(FName), GENERIC_READ, FILE_SHARE_READ, nil, OPEN_EXISTING, 0, 0);
...
begin
  Result := False;
  FileHandle := {$ifdef UNICODE}CreateFileW{$else}CreateFileA{$endif}(PChar(FileName), GENERIC_READ, FILE_SHARE_READ, nil, OPEN_EXISTING, 0, 0);
  if FileHandle = INVALID_HANDLE_VALUE then Exit;
