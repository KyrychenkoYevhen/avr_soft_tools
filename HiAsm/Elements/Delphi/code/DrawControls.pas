...
// Временно, пока не будет в порядке модуль Windows
{$ifdef UNICODE}
LOGFONT = LOGFONTW;
{$endif}
...
begin
  if(y < Rows)and(x < FColumns.Count)then
    PKOLStrList(FMatrix.Items[y]).Items[x] := value;
end;

function TDrawTable.GetCell;
begin
  if(y < Rows)and(x < FColumns.Count)then
    Result := PKOLStrListEx(FMatrix.Items[y]).Items[x];
end;

function TDrawTable.GetCellStyle;
begin
  if(y < Rows)and(x < FColumns.Count)then
    Result := PCellStyle(PKOLStrListEx(FMatrix.Items[y]).Objects[x]);
end;
...
begin
  if index = -1 then
   ...
   begin
      l := PKOLStrListEx(FMatrix.Items[index]);
