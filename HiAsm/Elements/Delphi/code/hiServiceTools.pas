...
function EnumServicesStatus(hSCManager: SC_HANDLE; dwServiceType,
  ...
  var pcbBytesNeeded, lpServicesReturned, lpResumeHandle: DWORD): BOOL; stdcall; external 'advapi32.dll' name {$ifdef UNICODE}'EnumServicesStatusW'{$else}'EnumServicesStatusA'{$endif};
function OpenSCManager(lpMachineName, lpDatabaseName: PChar;
  dwDesiredAccess: cardinal): SC_HANDLE; stdcall;  external 'advapi32.dll' name {$ifdef UNICODE}'OpenSCManagerW'{$else}'OpenSCManagerA'{$endif};
function OpenService(hSCManager: SC_HANDLE; lpServiceName: PChar;
  dwDesiredAccess: DWORD): SC_HANDLE; stdcall; external 'advapi32.dll' name {$ifdef UNICODE}'OpenServiceW'{$else}'OpenServiceA'{$endif};
function StartService(hService: SC_HANDLE; dwNumServiceArgs: DWORD;
  var lpServiceArgVectors: PChar): BOOL; stdcall; external 'advapi32.dll' name {$ifdef UNICODE}'StartServiceW'{$else}'StartServiceA'{$endif};
...
begin
  _hi_CreateEvent(_Data,@_event_onStatus, ServiceCntl(ReadString(_Data,_data_Name,_prop_Name), SERVICE_CONTROL_STOP));
end;
...
function RunProcess(Fn, CmdLine: string; ShowParams: Integer): Boolean;
var
  SI: TStartupInfo;
...
begin
  ...
  begin
    ...
    wShowWindow := ShowParams;
  end;
  
  if (Fn <> '') and (CmdLine <> '') then
    Fn := Fn + ' ' + CmdLine;
  
  if CreateProcess(nil, PChar(Fn), nil, nil,
                        False, CREATE_DEFAULT_ERROR_MODE, nil, nil{Working dir}, SI, PI)
  then
  begin
    Result := True;
...
begin
  ...
  if RunProcess(Fn, '/install', SW_HIDE) then
    _hi_CreateEvent(_Data, @_event_onExecService)
  else
    _hi_CreateEvent(_Data, @_event_onExecError, Integer(GetLastError)); 
end;
...
begin
  Fn := GetFullPath(ReadString(_Data, _data_FileName, _prop_FileName));
  if RunProcess(Fn, '/uninstall', SW_HIDE) then
    _hi_CreateEvent(_Data, @_event_onExecService)
  else
    _hi_CreateEvent(_Data, @_event_onExecError, Integer(GetLastError));      
end;
