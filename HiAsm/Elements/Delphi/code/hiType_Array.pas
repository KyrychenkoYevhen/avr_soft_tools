...
begin
  if (typ <> nil) and (ToInteger(item) < typ.count) then
    typ.NameOf[ToInteger(item)] := LowerCase(Share.ToString(val));
end;
...
begin
  dtNull(dt);
  if typ <> nil then typ.Add(LowerCase(Share.ToString(val)),@dt);
end;
...
begin
  if typ <> nil then
    if _prop_UseName then begin
      idx := typ.IndexOf(LowerCase(Share.ToString(item)));
...
begin
  if (typ <> nil) and (val.ldata <> nil) then typ.Add(LowerCase(Share.ToString(val)),val.ldata);
end;
