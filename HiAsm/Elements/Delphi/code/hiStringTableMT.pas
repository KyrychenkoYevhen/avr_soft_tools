...
uses
  Windows, ShellAPI, Kol, Messages, Share, Win, Debug, ListEdit;
...
type
  ThiStringTableMT = class(THIWin)
   private
    ...
    IDList:PKOLStrListEx;
    IList,IChList:PImageList;
    CList,FList:PKOLStrList;
    _StrDlm:char;
    _ColorDlm:char;
    _MTColDlm:char;
    ...
    procedure SetIcons(const value:PKOLStrListEx);
    procedure SetIconsCheck(const value:PKOLStrListEx);
    procedure SetMiscIcons(const value:PKOLStrListEx);
   ...
   public
    ...
    property _prop_Icons:PKOLStrListEx        write SetIcons;
    property _prop_IconsCheck:PKOLStrListEx   write SetIconsCheck;
    property _prop_MiscIcons:PKOLStrListEx    write SetMiscIcons;
...
var
  LVO: TListViewOptions;
  Ind: Integer;
begin
   Icon:= NewIcon;
   FFlat := _prop_Flat;
   LVO := [lvoUnderlineHot, lvoAutoArrange];

   if FInfoTip then include(LVO, lvoInfoTip);
   if FTrackSelect then include(LVO, lvoTrackSelect);
   if FHeaderDragDrop then include(LVO, lvoHeaderDragDrop);
   if FCheckBoxes then include(LVO, lvoCheckBoxes);
   if FMultiSelect then include(LVO, lvoMultiSelect);
   if FGrid then include(LVO, lvoGridLines);
   if FRowSelect then include(LVO, lvoRowSelect);
   if FFlat then include(LVO, lvoFlatsb);
   
   if not Assigned (IDList) then IDList:= NewKOLStrListEx;
   ...
   if FRedaction then
      if (Assigned (IChList)) and (FExtIconsCheck) then
           Control := NewListEdit(FParent,FStyle,LVO,IList,IList,IChList,_OnLineChange,_OnBeforeLineChange)
      else Control := NewListEdit(FParent,FStyle,LVO,IList,IList,nil,_OnLineChange,_OnBeforeLineChange)
   else
      if (Assigned (IChList)) and (FExtIconsCheck) then
           Control := NewListView(FParent,FStyle,LVO,IList,IList,IChList)
      else Control := NewListView(FParent,FStyle,LVO,IList,IList,nil);
