...
//    procedure _work_doLoad                 (var _Data: TData; Index: Word);
  ...
  protected                                   // коротких не вызывать авто - список
    ...
    FDictionary: PKOLStrListEx;                  // весь список слов, хранит номер последней вставки
...
begin
  ...
  for j := 0 to HS.Count - 1 do begin
    str := HS.Items[j];
    k := TypeArr[j];
    if k > 0 then begin // Комментарий
      if AnsiCmp(Copy(str, 1, k-1)) then begin
        SetColorStr;
        Result := PosEx(Copy(str, k+1, Length(str)), S, i+k)
                + Length(str)-k-1 - FromPos.x;
        exit; // Возвращаем комментарий
      end;
    end else if AnsiCmp(str) then begin
      SetColorStr;
      inc(i, Length(str)); // Если Блок - заканчиваем сразу
      if k = 0 then // Если Слово - дополняем до первого пробела
        while (i<Length(S))and(S[i+1]>' ') do inc(i);
      Result := i - FromPos.x;
      exit; // Возвращаем либо Блок, либо просто Слово
    end;
  end;
  // Ничего не нашли - пропускаем ОДИН символ, и - возможные после него пробелы
