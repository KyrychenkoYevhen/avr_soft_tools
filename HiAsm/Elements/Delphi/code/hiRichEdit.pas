...
uses
  Windows, Messages, Kol, Share, EWinList;
...
begin
   Control.SelStart := 0;
   Control.SelLength := High(Integer);
...
begin
   _debug('This method not supported');
end;
{$else}
procedure THIRichEdit._work_doAddBitmap;
var bmp:kol.PBitmap;
...
begin
   dtString(_Data, Control.RE_Text[{$ifdef UNICODE}reTextUnicode{$else}reText{$endif}, False]);
end;
