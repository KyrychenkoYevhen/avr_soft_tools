...
var list:string;
 ...
 procedure WVar(typ:PType; list:PKOLStrList; offset:string);
 var s,str:string;
     i,n:integer;
     dt:PData;
     tp:PType;
 begin
   for i := 0 to typ.count-1 do begin
     ...
     case dt.data_type of
       ...
       list.items[n] := list.items[n] + ReplaceSign(Share.ToString(dt^)) + '</' + s + '>';
     end;
   end;
 end;
  ...
  if _prop_StreamFormat = 0 then begin
    ...
    if typ <> nil then begin
      ...
      for i := 0 to typ.count-1 do 
        list.add(typ.NameOf[i] + '=' + Share.ToString(typ.data[i]^));
    end else CallTypeError('',_event_onError,TYPE_ERR_INVALID_TYPE);
    if fn = '' then begin
