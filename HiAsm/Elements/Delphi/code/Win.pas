...
uses
  Windows, ShellAPI, Messages, KOL, {KOLMHToolTip,}Share, Debug, hiHintManager, WinLayout;
...
type
 THIWin = class(TDebug)
   protected
     ...
     function  _onMessage( var Msg: TMsg; var Rslt: LRESULT ): Boolean; virtual;
...
var  ListOfNameControls: PKOLStrListEx;
...
begin
  ...
  begin
      ...
      if _prop_Ctl3D = 0 then Ctl3D := True; // Устанавливаем только при _prop_Ctl3D = True
...
//        _prop_Layout.addControl(Control, _prop_WidthScale, _prop_HeightScale);
      ...
      if (_prop_WinStyle > 0) and (_prop_WinStyle < Cardinal(Length(WinStyle_ExSet))) then
      begin
        Control.ExStyle := WinStyle_ExSet[_prop_WinStyle];
      end;
  end;
  if _prop_Name <> '' then ListOfNameControls.AddObject(LowerCase(_prop_Name), NativeUInt(Control));
end;
...
begin
   if assigned(_prop_HintManager) then _prop_HintManager.hint(hid, Share.ToString(Data));
end;
{
...
var
  ...
  R: TRect;
begin
  if Control.IsForm then
  begin
    if not (
      (Control.Parent <> nil) and
      (Control.Parent.IsForm) and
      GetWindowRect(Control.Parent.Handle, R)
    ) then
    begin
      R.Left := 0;
      R.Right := ScreenWidth;
  ...
  begin
    ...
    if (H = 0) or (not GetClientRect(H, R)) then Exit;
  end;
  Control.Left := R.Left + ((R.Right - R.Left) - Control.Width) div 2;
  Control.Top := R.Top + ((R.Bottom - R.Top) - Control.Height) div 2;
end;
...
begin
   dtInteger(Data, NativeInt(Control) );
end;
...
initialization
   ...
   ListOfNameControls := NewKOLStrListEx;
