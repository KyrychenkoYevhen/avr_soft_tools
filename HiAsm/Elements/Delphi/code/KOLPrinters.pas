...
uses
  Windows, Messages, KOL, KOLPrintCommon, Share;
...
type
  THookProc = function(Wnd: HWND; Message: UINT; wParam: WPARAM; lParam: LPARAM): NativeUInt; stdcall;
  ...
  tagPDA = record
    ...
    hInstance: HINST;
    lCustData: LPARAM;
    lpfnPrintHook: THookProc;
    lpfnSetupHook: THookProc;
  ...
  tagPD = {$ifdef UNICODE}tagPDW{$else}tagPDA{$endif};
  PtagPD = ^tagPD;
...
function PrintDlg(var PrintDlg: tagPD): BOOL; stdcall;external 'comdlg32.dll' name {$ifdef UNICODE}'PrintDlgW'{$else}'PrintDlgA'{$endif};
...
begin
       Len := 1;
       while Windows.{$ifdef UNICODE}DrawTextW{$else}DrawTextA{$endif}(fCanvas.Handle,pC,Len,Rect,DT_CALCRECT + DT_WORDBREAK + DT_NOCLIP + DT_NOPREFIX + DT_EXPANDTABS) < PageH do
       begin
     ...
     (if possible) on bottom-right corner of the page.Do it only for multipage text (Len<>Size)  }
   {
    ...
    { Finally draw it!}
    Windows.{$ifdef UNICODE}DrawTextW{$else}DrawTextA{$endif}(fCanvas.Handle,pC,Len,Rect,DT_WORDBREAK + DT_NOCLIP + DT_NOPREFIX + DT_EXPANDTABS);
...
begin
    ...
    fDeviceMode  := PDeviceMode(GlobalLock(fDevMode));
    try
...
begin
    ...
    if Source = nil then DefPrinter
        ...
        DevMode := PDeviceMode(GlobalLock(Source^.ADevMode));
        try
            Size := sizeof(DevMode^);
...
begin
    ...
    fPageNumber :=1;
    fPrinting := True;
    FillChar(doc,sizeof(TDocInfo),0);
