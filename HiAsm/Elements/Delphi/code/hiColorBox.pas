...
uses
  Windows, KOL, Share, LWinList, Messages, hiBoxDrawManager;
...
function Trim(S: string; D: Char = ' '): string;
var
  I, L: Integer;
begin
  Result := '';
  ...
  begin
    ...
    while (I <= L) and (S[I] = D) do Inc(I);
    while (L >= I) and (S[L] = D) do Dec(L);
  ...
  case _prop_DataType of
    ...
    1: begin
         case fFormatColor of
           ...
           3: begin
                fFrom := TRGB(Integer(Control.ItemData[Control.CurIndex]));
...
begin
  Idx := ToIntIndex(Item);
  Result := (Idx >=0) and (Idx < Control.Count);
  if Result then
    dtInteger(Val,Control.ItemData[Idx]);
end;
...
var
  Clr: string;
begin
  Clr := Share.ToString(_Data);
  Replace(Clr, 'cl', '');
  Control.Perform(CB_SELECTSTRING, WPARAM(-1), LPARAM(PChar(Clr)));
end;
