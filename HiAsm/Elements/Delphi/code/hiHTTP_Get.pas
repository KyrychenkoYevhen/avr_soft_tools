...
type
  THIHTTP_Get = class(TDebug)
   ...
   procedure _work_doUserAgent(var _Data:TData; Index:word);
...
begin
  ...
  hFile := InternetOpenURL(hSession, PChar(FileURL),nil,0,INTERNET_FLAG_RELOAD+INTERNET_FLAG_NO_CACHE_WRITE+INTERNET_FLAG_NO_AUTH, 0);
  if Assigned(hFile) then
  begin 
    dwIndex := 0;
    dwBufferLen := 20;
    if HttpQueryInfo(hFile, HTTP_QUERY_CONTENT_LENGTH, @dwBuffer[0], dwBufferLen, dwIndex) then Result := str2int(dwBuffer);
   ...
   begin
     ...
     if _prop_Method = 1 then
       begin
         ...
         UrlHandle := HttpOpenRequest(ConHandle,'POST',PChar(s1),nil,nil,nil,INTERNET_FLAG_KEEP_CONNECTION or dwStatusSize,0);
         s := 'Content-Type: application/x-www-form-urlencoded';  
         s1 := ReadString(GData, _data_PostData);
         HttpSendRequest(UrlHandle, PChar(s), length(s), PAnsiChar(s1), Length(s1));
       end
     else
        UrlHandle := InternetOpenUrl(NetHandle, PChar(Url), PChar(Head), cardinal(-1), INTERNET_FLAG_RELOAD+INTERNET_FLAG_NO_CACHE_WRITE+INTERNET_FLAG_NO_AUTH, 0);
...
//     else ;//MessageBox(0,'Can''t open URL!','Error',MB_OK);
     ...
     if Assigned(ConHandle) then InternetCloseHandle(ConHandle);
...
procedure THIHTTP_Get._work_doUserAgent;
begin
  _prop_UserAgent := Share.ToString(_Data);
end;
