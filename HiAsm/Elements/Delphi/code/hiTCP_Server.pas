...
begin
    case _prop_DataType of
     0: begin Share.dtInteger(dt, NativeInt(buf^)); event(); end;
     1: begin
          s := BufToBinaryStr(Buf, Count);
          Share.dtString(dt, s);
          event();
         end;
       ...
       begin
         if Mem = nil then
          begin
           ...
           CopyMemory(pointer(NativeInt(@FSize) + FSizeCount), buf, c);
           inc(FSizeCount, c);
   ...
   if Sock.Connected then
    for i := 0 to Sock.Count-1 do
     case _prop_DataType of
       ...
       1: Sock.Connections[i].Send(Pointer(s), BinaryLength(s));
...
begin
   ...
   if Sock.Connected and (con <> nil) then
     case _prop_DataType of
       ...
       1: con.Send(Pointer(s), BinaryLength(s));
...
begin
  Sock.DisconnectByIP(Share.ToString(_Data));
end;
