...
var   objService : OLEVariant;
      objProcessor: OLEVariant; 
      colProcessor: OLEVariant;
      oEnum : IEnumVariant;
      iValue : LongWord;
      sComputer : string;      
begin
  ...
  iValue := 0;
  while oEnum.Next(1,objProcessor,{$ifndef FPC_NEW}@{$endif}iValue) = 0 do 
  begin
...
begin
  ...
  colProcessor := objService.ExecQuery(WideString('SELECT ' + sQuery + ' FROM Win32_Processor' + sDevice));
  oEnum := IUnknown(colProcessor._NewEnum) as IEnumVariant;
  iValue := 0;
  while oEnum.Next(1,objProcessor,{$ifndef FPC_NEW}@{$endif}iValue) = 0 do 
  begin
  {$ifdef FPC_NEW}
    try DeviceID          := Trim(VarToStr(objProcessor.DeviceID)); except DeviceID := '' end;
    try Name              := Trim(VarToStr(objProcessor.Name)); except Name := '' end;
    try Manufacturer      := Trim(VarToStr(objProcessor.Manufacturer)); except Manufacturer := '' end;
    try Description       := Trim(VarToStr(objProcessor.Description)); except Description := '' end;
    try SocketDesignation := Trim(VarToStr(objProcessor.SocketDesignation)); except SocketDesignation := '' end;
    try MaxClockSpeed     := Trim(VarToStr(objProcessor.MaxClockSpeed)); except MaxClockSpeed := '' end;
    try ExtClock          := Trim(VarToStr(objProcessor.ExtClock)); except ExtClock := '' end;
    try Version           := Trim(VarToStr(objProcessor.Version)); except Version := '' end;
    try L2CacheSize       := Trim(VarToStr(objProcessor.L2CacheSize)); except L2CacheSize := '' end;
    try L2CacheSpeed      := Trim(VarToStr(objProcessor.L2CacheSpeed)); except L2CacheSpeed := '' end;
    
    try
      if not VarIsEmpty(objProcessor.Architecture) then
      begin
    ...
    try 
      if not VarIsEmpty(objProcessor.Availability) then 
      begin
    ...
    try ProcessorId       := Trim(VarToStr(objProcessor.ProcessorId)); except ProcessorId := '' end;
    try Status            := Trim(VarToStr(objProcessor.Status)); except Status := '' end;
    try LoadPercentage    := Trim(VarToStr(objProcessor.LoadPercentage)); except LoadPercentage := '' end;
