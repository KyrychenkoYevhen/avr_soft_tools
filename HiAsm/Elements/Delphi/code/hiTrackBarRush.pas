...
function WndTrackBarExProc(Sender: PControl; var Msg: TMsg; var Rslt: LRESULT): Boolean;
var
  HiClass: THITrackBarRush;
  point : TPoint;
  t_MouseEvent: TTrackMouseEvent;
  oldAbsPos, AbsPos, Pos, pos1: Integer;
begin
  Result := FALSE;
  ...
  with HiClass do
    case msg.Message of
      ...
      //---------------------------начало правки от 3042 от 16.07.2019----------------------------
      WM_KEYDOWN:
        if Sender.Focused then
        begin
          pos1 := FPosition; //Запоминаем
          case Msg.WParam of
            ...
            else exit; //Чтоб не срабатывало при нажатии "левых" клавиш
          end;
          SetPosition(FPosition);
          if _prop_AbsPosition then
            begin
             _hi_onEvent(_event_onStart, abs(FPosition));
             if pos1 <> FPosition then _hi_onEvent(_event_onPosition, abs(FPosition))
            end
          else
           begin
            _hi_onEvent(_event_onStart, FPosition);
            if pos1 <> FPosition then _hi_onEvent(_event_onPosition, FPosition)
           end
        end;
      WM_KEYUP:
        if Sender.Focused then
          case Msg.WParam of //тоже от "левых" клавиш
            VK_LEFT, VK_UP, VK_PAGE_UP,
            VK_RIGHT, VK_DOWN, VK_PAGE_DOWN,
            VK_END, VK_HOME:
             if _prop_AbsPosition then
               _hi_onEvent(_event_onStop, abs(FPosition))
             else
               _hi_onEvent(_event_onStop, FPosition)
          end;
      ...
      WM_LBUTTONDOWN:
        begin
          ...
          if PointInRect(Point, FThumbRect) then //Нажатие на ползунке
          begin
            SetCapture(Sender.Handle);
            FThumbState:= bsDown;
            PaintTrackbar;
          end
          else if PointInRect(Point, FSlideRect) then //Нажатие на трекбаре вне ползунка
          begin
            oldAbsPos := FAbsPos;
            if FOrientation = trHorizontal then
	  ...
	  //---------------------------конец правки от 3042 от 16.07.2019----------------------------
      WM_MOUSEMOVE:
        begin
