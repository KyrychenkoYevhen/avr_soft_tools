...
function WndProcGLabel( Sender: PControl; var Msg: TMsg; var Rslt: LRESULT ): Boolean;
var
  HiClass: ThiGLabel;
begin
  Result := false;
...
begin
   ...
   with FBackBmp.Canvas{$ifndef F_P}^{$endif} do begin
      ...
      Windows.{$ifdef UNICODE}DrawTextW{$else}DrawTextA{$endif}(Handle, PChar(FCaption), -1, BRect, _Flags);
      ...
      SetBkMode(Handle, Windows.TRANSPARENT);
      RequiredState( HandleValid or FontValid or BrushValid or ChangingCanvas );
      Windows.{$ifdef UNICODE}DrawTextW{$else}DrawTextA{$endif}(Handle, PChar(FCaption), -1, R0, _Flags);
