...
const
  CP_ACP   =  0; // Default system ANSI page
...
uses
  hiCharset, hiConvertor, CodePages;
...
procedure THIPOP3_Parse.Decode(_Type,_Encode:string; const Text:string);
var s,_Content_Type,_FName,_Char:string;
  procedure _Decode(var Result:string);
  var lst:PKOLStrList; i:integer;
  begin
...
//        Result := Koi8ToWin(Result)
          Result := ConvertCharset(Result, CP_KOI8, CP_ACP)
      else if StrIComp(PChar(_Char),'ibm866') = 0 then
//        Result := DosToWin(Result);
          Result := ConvertCharset(Result, CP_DOS, CP_ACP);
    end;
  end;
  ...
  while i < Length(s) do begin
    if (s[i]='=')and(s[i+1]='?') then begin
      ...
      if StrIComp(PChar(ch),'koi8-r') = 0 then
        name := ConvertCharset(name, CP_KOI8, CP_ACP)
//        name := Koi8ToWin(name)
      else if StrIComp(PChar(ch),'ibm866') = 0 then
          name := ConvertCharset(name, CP_DOS, CP_ACP);
//        name := DosToWin(name);
  ...
  if(s = 'multipart/mixed')or(s = 'multipart/alternative')then begin
    ...
    while i < list.Count do begin
      ...
      if StrIComp(PChar(Content_Type),'koi8-r') = 0 then
          s := ConvertCharset(s, CP_KOI8, CP_ACP)
//        s := Koi8ToWin(s)
      else if StrIComp(PChar(Content_Type),'ibm866') = 0 then
          s := ConvertCharset(s, CP_DOS, CP_ACP);
//        s := DosToWin(s);
