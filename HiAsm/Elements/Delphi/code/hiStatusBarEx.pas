...
function WndProcResize( Sender: PControl; var Msg: TMsg; var Rslt: LRESULT ): Boolean;
begin
  Result := false;
...
begin
  ...
  begin
    ...
    if not _prop_Ctl3DPBar then Progress.Ctl3D := False;
    Progress.Height        := Control.StatusCtl.Height - 8;
    Progress.Top           := Control.StatusCtl.Top + 5;
...
begin
  if not Assigned(Progress) then exit; 
  _prop_HintPBar:= Share.ToString(_Data);
  s := TempText;
  Replace(s, GSymbol, '');
  if (_prop_HintPBar <> '') and  not (_prop_PanelHintPBar) then
    SetHint(_prop_HintPBar)
