...
function decode_xor(const data, key:string):string;
var i,j:integer;
    r:string;
begin
   j := 1;
   ...
   for i := 1 to length(data) do
     begin
       r[i] := chr(ord(data[i]) xor ord(key[j]));
...
//-------------------------------------------------------------
     ...
     begin
       UnitName := cgt.ReadCodeDir(el) + 'hi' + cgt.elGetClassName(e) + '.hes';
       
       fs := NewReadFileStream(UnitName);
       fs.Read(len, 2);
       SetLength(buf, len);
       fs.Read(buf[1], len);
    
       key_fs := NewReadFileStream(cgt.ReadCodeDir(el) + buf + '.key');
       SetLength(key, key_fs.Size);
       key_fs.Read(key[1], key_fs.Size);
       key_fs.Free;
       key := decode_xor(key, '1234-qwer-0987-asdf');
       ...
       i := pos('---', key);
       if MD5DigestToStr(MD5String(GetExtendedCpuName)) = copy(key, 1, i-1) then
         begin
           delete(key, 1, i + 2);
           Lines.Text := decode_xor(data, key + ' ')
         end
       else
         AddError('Wrong architecture for the specified key: "' + buf + '.key"');
     end;
//-------------------------------------------------------------
