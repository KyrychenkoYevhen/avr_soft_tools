...
type
  ...
  public
    ...
    {*}
    property CMYK: Boolean read FCMYK;
    {*}
    ...
    function LoadFromFile( const FilName : String ) : Boolean;
    {* Function to load jpeg image from a file. }
    function SaveToFile( const FilName : String ) : Boolean;
...
type
  ...
  PProgressExtraInfo = ^TProgressExtraInfo;
...
begin
  asm
    ...
    fstp  qword ptr [{$IFDEF WIN64}rax{$ELSE}eax{$ENDIF}]     //  Delphi passes floats on the CPU stack
  end;
...
procedure FormatMessage(cinfo: j_common_ptr; {$IFDEF FPC}var buffer: string{$ELSE}buffer: PChar{$ENDIF});
begin
  //!!
end;
...
var
  ...
  R: TRect;
  temp: Integer;
  {$IFDEF FPC}EI:PProgressExtraInfo;{$ENDIF}
begin
  {$IFDEF FPC}
  if (cinfo.progress = nil) or (cinfo.client_data = nil) or (TProgressExtraInfo(cinfo.client_data^).instance = nil) then Exit;
  EI := PProgressExtraInfo(cinfo.client_data);
  with cinfo.progress^ do
  begin
    Ticks := GetTickCount;
    if (Ticks - EI.last_time) < EI.instance.fProgressTime then Exit;
    temp := EI.last_time;
    EI.last_time := Ticks;
    if temp = 0 then Exit;
    if cinfo.is_decompressor then
      ...
      begin
        R := MakeRect(0, EI.last_scanline, output_width, output_scanline);
        if R.Bottom < EI.last_scanline then
          R.Bottom := output_height;
      end
    else
      R := MakeRect(0,0,0,0);
    temp := Integer(Trunc(100.0*(completed_passes + (pass_counter/pass_limit))/total_passes));
    if temp = EI.last_pct then Exit;
    EI.last_pct := temp;
    if cinfo.is_decompressor then
      EI.last_scanline := j_decompress_ptr(@cinfo)^.output_scanline;
    //instance.Progress(instance, psRunning, temp, (R.Bottom - R.Top) >= 4, R, '');
    if EI.instance.FProgress( EI.instance, R ) then
      EI.instance.FStop := TRUE;
  end;
...
begin
  ...
  jpeg_destroy({$IFDEF FPC}@{$ENDIF}jc.common);
  jc.common.err := nil;
end;
...
begin
  ...
  jpeg_CreateDecompress({$IFDEF FPC}@{$ENDIF}jc.d, JPEG_LIB_VERSION, sizeof(jc.d));
  with Obj^ do
  try
    jc.progress.progress_monitor := FCallback;
    {$IFDEF FPC}
    jc.d.client_data := new(PProgressExtraInfo);
    TProgressExtraInfo(jc.d.client_data^).instance := Obj;
    {$ELSE}
    ...
    jpeg_stdio_src({$IFDEF FPC}@{$ENDIF}jc.d, FImage.FData);
    jpeg_read_header({$IFDEF FPC}@{$ENDIF}jc.d, TRUE);
...
//      jc.d.do_fancy_upsampling := False;    !! AV inside jpeglib
    ...
    if FProgressiveDisplay and jpeg_has_multiple_scans({$IFDEF FPC}@{$ENDIF}jc.d) then
...
begin
  ...
  try
    ...
    jpeg_calc_output_dimensions({$IFDEF FPC}@{$ENDIF}jc.d);
    // read output dimensions
    FScaledWidth := jc.d.output_width;
    FScaledHeight := jc.d.output_height;
    FProgressiveEncoding := jpeg_has_multiple_scans({$IFDEF FPC}@{$ENDIF}jc.d);
  finally
    ReleaseContext(jc);
  end;
end;
...
begin
  ...
  jpeg_CreateCompress({$IFDEF FPC}@{$ENDIF}jc.c, JPEG_LIB_VERSION, sizeof(jc.c));
  try
    try
      jc.progress.progress_monitor := FCallback;
      {$IFDEF FPC}
      jc.d.client_data := new(PProgressExtraInfo);
      TProgressExtraInfo(jc.d.client_data^).instance := @Self;
      {$ELSE}
      ...
      jpeg_stdio_dest({$IFDEF FPC}@{$ENDIF}jc.c, FImage.FData);
      ...
      try
        ...
        jpeg_set_defaults({$IFDEF FPC}@{$ENDIF}jc.c);
        jpeg_set_quality({$IFDEF FPC}@{$ENDIF}jc.c, FQuality, True);

        if FGrayscale then
        begin
          FImage.FGrayscale := True;
          jpeg_set_colorspace({$IFDEF FPC}@{$ENDIF}jc.c, {$IFDEF FPC}jpeglib.{$ENDIF}JCS_GRAYSCALE);
        end;

        if ProgressiveEncoding then
          jpeg_simple_progression({$IFDEF FPC}@{$ENDIF}jc.c);
        ...
        try
          jpeg_start_compress({$IFDEF FPC}@{$ENDIF}jc.c, True);

          while (jc.c.next_scanline < jc.c.image_height) do
          begin
            LinesWritten := jpeg_write_scanlines({$IFDEF FPC}@{$ENDIF}jc.c, @SrcScanline, LinesPerCall);
            Inc(NativeInt(SrcScanline), PtrInc * LinesWritten);
          end;

          jpeg_finish_compress({$IFDEF FPC}@{$ENDIF}jc.c);
        finally
          {--
...
begin
  ...
  cinfo.colormap := {$IFDEF FPC}cinfo.mem.alloc_sarray(@cinfo, JPOOL_IMAGE, Count, 3);{$ELSE}cinfo.common.mem.alloc_sarray(@cinfo.common, JPOOL_IMAGE, Count, 3);{$ENDIF}
...
begin
  ...
  try
    try
      ...
      //--Progress(Self, psStarting, 0, False, Rect(0,0,0,0), '');
      if FProgress( @Self, MakeRect( 0, 0, 0, 0 ) ) then Exit;
      ...
      try
        ...
        jpeg_start_decompress({$IFDEF FPC}@{$ENDIF}jc.d);
        //{$IFNDEF JPEGERR}
        ...
        begin  // decode progressive scans at low quality, high speed
          while jpeg_consume_input({$IFDEF FPC}@{$ENDIF}jc.d) <> JPEG_REACHED_EOI do
          begin
            jpeg_start_output({$IFDEF FPC}@{$ENDIF}jc.d, jc.d.input_scan_number);
            {$IFNDEF JPEGERR}
            if Jpeg_Error then Exit;
            {$ENDIF}
            if FStop then Exit;
            ...
            begin
              LinesRead := jpeg_read_scanlines({$IFDEF FPC}@{$ENDIF}jc.d, @DestScanline, LinesPerCall);
              Inc(NativeInt(DestScanline), PtrInc * LinesRead);
            end;
            jpeg_finish_output({$IFDEF FPC}@{$ENDIF}jc.d);
          end;
        ...
        if jc.d.buffered_image then jpeg_finish_output({$IFDEF FPC}@{$ENDIF}jc.d);
        jpeg_finish_decompress({$IFDEF FPC}@{$ENDIF}jc.d);
        OK := TRUE;
      finally
        {--
        if ExceptObject = nil then
  ...
  try
    ...
    begin
      ...
      begin
        ...
        {$IFDEF FPC}cinfo.err{$ELSE}cinfo.common.err{$ENDIF} := @jerr;
        jpeg_CreateDecompress({$IFDEF FPC}@{$ENDIF}cinfo, JPEG_LIB_VERSION, sizeof(cinfo));
        try
          FData.Position := 0;
          jpeg_stdio_src({$IFDEF FPC}@{$ENDIF}cinfo, FData);
          jpeg_read_header({$IFDEF FPC}@{$ENDIF}cinfo, TRUE);
          FWidth := cinfo.image_width;
          FHeight := cinfo.image_height;
          FGrayscale := cinfo.jpeg_color_space = JCS_GRAYSCALE;
          FProgressiveEncoding := jpeg_has_multiple_scans({$IFDEF FPC}@{$ENDIF}cinfo);
        finally
          jpeg_destroy_decompress({$IFDEF FPC}@{$ENDIF}cinfo);
        end;
      end;
    end;
    //PaletteModified := True;
...
function TJpeg.LoadFromFile( const FilName : String ) : Boolean;
var Strm : PStream;
begin
  Clear;
  Strm := NewReadFileStream( FilName );
  if Strm.Size > 0 then
     LoadFromStream( Strm );
  Strm.Free;
  Result := not Empty;
...
function TJpeg.SaveToFile( const FilName : String ) : Boolean;
var Strm : PStream;
begin
  Result := False;
  if Empty then Exit;
  Strm := NewWriteFileStream( FilName );
  SaveToStream( Strm );
...
function TJpeg.GetHasBitmap: Boolean;
begin
  Result := FBitmap <> nil;
end;
