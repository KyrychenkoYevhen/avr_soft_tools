...
begin
  ...
  begin
    ...
    {$IFDEF WIN64}
    {$ifndef DELPHI64} // 64-bit Delphi does not support mixing Pascal and Asm code in functions
    // TODO: Rewrite in Pascal
    asm
          PUSH  RSI
          PUSH  RDI
          MOV   RSI, FirstItemPtr
          MOV   RDI, RSI
          STD
          MOV   RCX, N
          XOR   RAX, RAX
...
begin
  ...
  begin
    ...
    begin
      ...
      if D <> 0 then
      begin
        {$ifndef DELPHI64} // TODO: Rewrite in Pascal
        asm
          MOV  EAX, D
          BSF  EAX, EAX
          MOV  D, EAX
...
function TDirChange.Execute(Sender: PThread): NativeInt;
asm
        PUSH     EBX
        PUSH     ESI
        XCHG     EBX, EAX
...
begin
  ...
  pW := @WMF;
  pEnd := @WMF.CheckSum;
  while NativeInt(pW) < NativeInt(pEnd) do
  begin
    Result := Result xor pW^;
    Inc(NativeInt(pW), SizeOf(Word));
  end;
end;
