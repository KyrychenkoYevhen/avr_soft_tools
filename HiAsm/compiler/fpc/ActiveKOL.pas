...
uses
  windows, messages, KOL, ActiveX, KOLComObj{, err}, Variants;
...
type
  ...
  protected
    ...
    function TranslateAccelerator(var msg: TMsg;
  ...
  TOleCtl = class(TControl)
  private
    function GetOleObject: IDispatch;
  ...
  public
    ...
    property  OleObject: IDispatch read GetOleObject;
...
const
  SInvalidLicense = 'Invalid license.';
  SNotLicensed = 'Not licensed.';
...
begin
  for I := 0 to FValueCount - 1 do
    with FValues^[I] do Proc(int2str(Value)+' - '+Ident);
end;
  ...
  with FControlData^ do
    ...
    begin
      for I := 0 to EnumPropDescs.Count - 1 do
        PEnumPropDesc(EnumPropDescs.Items[I]).Destroy;
      EnumPropDescs.Free;
      EnumPropDescs := nil;
    end;
end;
...
function TOleCtl.GetOleObject: IDispatch;
begin
  CreateControl;
...
begin
  Result := Variant(GetOleVariantProp(Index));
end;
...
procedure TOleCtl.KeyPress(var Key: Char);
begin
...
begin
  SetOleVariantProp(Index, OleVariant(Value));
end;
...
function TOleCtlIntf.TranslateAccelerator(var msg: TMsg;
  wID: Word): HResult;
begin
  Result := S_FALSE;
end;
