...
function StringToOleStr(const s:string): POleStr;
procedure OleStrToStrVar(const strFrom:POleStr; var strTo:string);

procedure VarClear(var V:Variant);
procedure VarCast(const Vfrom:Variant; var Vto:Variant; ToType:integer);
...
begin
  ...
  begin
    ...
    try
      CreateRegKey(ClassKey + '\Version', '', int2str(TLibAttr.wMajorVerNum) + '.' + int2str(TLibAttr.wMinorVerNum));
...
var DestLen:integer;
begin
  DestLen := WideCharToMultiByte(0, 0, strFrom, -1, nil, 0, nil, nil);
  if DestLen=1 then strTo:='' else begin
    SetLength(strTo, DestLen-1);
    WideCharToMultiByte(0, 0, strFrom, -1, @strTo[1], DestLen-1, nil, nil);
  end;
end;

procedure VarClear(var V:Variant);
begin
  VariantClear(OleVariant(V));
end;

procedure VarCast(const Vfrom:Variant; var Vto:Variant; ToType:integer);
begin
  VariantChangeType(OleVariant(Vto),OleVariant(Vfrom),0,ToType);
end;
...
procedure DispCall(const Dispatch: IDispatch; CallDesc: PCallDesc;
  DispID: Integer; NamedArgDispIDs, Params, PResult: Pointer); stdcall;
type
...
@@12:   MOVZX   ECX,[EBX].TCallDesc.CallType
        ...
        INC     DispParams.cNamedArgs
        CMP     BYTE [EBX].TCallDesc.ArgTypes[0],varDispatch
        JE      @@13
        CMP     BYTE [EBX].TCallDesc.ArgTypes[0],varUnknown
        JNE     @@20
...
procedure DispCallByID(PResult: Pointer; PDispatch: pointer;
